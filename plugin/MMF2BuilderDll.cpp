...
// ------------------------------------------------------
// Modifications made by shishkabob for use with NuclearRT.
// ------------------------------------------------------
...
BOOL WINAPI Build (LPCWSTR pTargetPathname, LPCWSTR pCCNPathname, int idx, DWORD buildFlags)
{
    ...
    swprintf(directory, MAX_PATH, L"%s%s", tempdirectory, L"\\Data\\Runtime\\NuclearRT\\exporter\\");
    wchar_t exe_path[MAX_PATH];
    swprintf(exe_path, MAX_PATH, L"%s%s", directory, L"exporter.exe");
    ...
    else
    {
        ...
        // Display message.
        MessageBox(NULL, Error, "Couldn't open NuclearRT build process", MB_OK|MB_ICONWARNING );
